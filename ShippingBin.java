...
public class ShippingBin {
    ...
    public void addItem(Items item, int quantity, Player player) {
        ...
        if (!player.haveitem(item) || player.getInventory().getItemsMap().get(item) < quantity) {
    ...
    public void sellBin(Player player) {
        ...
        // Hitung total harga jual dari semua item dalam bin
        // yang punya harga jual hanya crops dan fish
        // item lain tidak bisa dijual

        int totalSellPrice = bin.entrySet().stream()
            .mapToInt(entry -> entry.getKey().getSellPrice() * entry.getValue())
            .sum();

        player.setGold(player.getGold() + totalSellPrice);
